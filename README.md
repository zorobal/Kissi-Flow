...
*   **👑 Cockpit SuperAdmin & Synchronisation Cloud (Supabase) & Planification Réseau** : Module de supervision multi-site hautement sécurisé pour les franchises et groupes. Permet de consolider l'ensemble des KPIs (CA consolidé, TVA cumulée, charges, résultat net, AOV, marge nette %, couverts, expense ratio). Il intègre un outil de détection automatique de sites actifs sur le cloud Supabase, d'import/export de l'ensemble des 19 collections locales, d'une option de saisie manuelle de l'identifiant pour la portabilité absolue, et d'un journal de logs rétro-terminal en temps réel. **Nouveau :** Intègre un planificateur et simulateur interactif de topologies réseau permettant de configurer et tester en direct les 4 scénarios d'infrastructures physiques (Cloud en ligne, PostgreSQL local sur LAN, Serveur Web local centralisé, Tauri Desktop SQLite autonome) avec calcul d'indices de faisabilité opérationnelle. L'accès aux configurations de synchronisation cloud dans les paramètres est restreint de manière sécurisée au profil `SUPERADMIN`.
